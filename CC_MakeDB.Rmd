...
Casual conversation seems to occur in phases or bouts, thus sharing the load among participants. 
This can take the form of discussions where two or more participants contribute significantly, or 'long turns' where one participant dominates the conversation, often through narrative or giving an extended explanation/opinion. These one participant dominated bouts will hencforth be termed 'dominated'.

I would like to know if these 'bouts', particularly the dominated bouts, have characteristic patterns of silence and speech.
...
First, read in text file (SOC - speech, overlap, chunks)

```{r}
cc_df <- read.csv("BESTENDtest.txt",header=F,sep=',',stringsAsFactors = F)
```
