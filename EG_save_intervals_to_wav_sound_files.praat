...
# adapted from a script Copyright 8.3.2002 Mietta Lennes
#

form Save intervals to small sound files with resampling to prep for Penn Aligner
	comment Each interval in the selected tier will be saved to a separate WAV sound file which will be resampled.
	comment Each WAV will be resampled to the desired sampling rate
	comment The sound files will be numbered and the labels will be saved in a text file, one label per line.
	integer Resampling_rate 11025
...
for interval from start_from to end_at
	...
	intlabel$ = Get label of interval... tier interval
	check = 0
...
# 		intervalstart = Get starting point... tier interval
		...
		intervalfile$  = "'folder$'" + "'prefix$'" + "'intervalstart'" +"_" + "'tiername$'" +"_" + "'intervalend'"+ ".wav"
		Write to WAV file... 'intervalfile$'
		Remove
		select Sound 'soundname$'
		Remove
		# Now open a textfile for this wav, take the label of the saved sound interval and write it into textfile:
		intervaltextfile$ = "'folder$'" + "'prefix$'" + "'intervalstart'" +"_" + "'tiername$'" +"_" + "'intervalend'"+ ".txt"
		select TextGrid 'soundname$'
		intline$ = "'intlabel$'" + "'newline$'"
		fileappend "'intervaltextfile$'" 'intline$'
	endif
endfor
